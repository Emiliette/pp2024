...
class Student:
    def __init__(self, stdscr):
        self.__id = self.get_input(stdscr, "\nEnter student ID: ")
...
class University:
    ...
    def set_num_students(self):
        ...
        curses.echo()
        self.__num_students = int(self.stdscr.getstr().decode("utf-8"))
        curses.noecho()
    
    def set_num_courses(self):
        self.stdscr.addstr("\nEnter number of courses: ")
        curses.echo()
        self.__num_courses = int(self.stdscr.getstr().decode("utf-8"))
        curses.noecho()
    
    def add_student(self):
        if len(self.__students) < self.__num_students:
    ...
    def add_course(self):
        if len(self.__courses) < self.__num_courses:
